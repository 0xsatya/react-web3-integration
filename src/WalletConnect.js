...
//   }, [])
  ...
  return (
    <div>
        ...
        <p>
        <Button variant="primary" name='connect wallet' onClick={connectWallet} >connectWallet</Button>
        </p>
        <div>
          <span>{
            personalWallet ?
                <div>
                    <h3> Connect Wallet Id: {personalWallet}</h3>
                    <h3> Wallet Balance: {personalWalletBalance} </h3>
                    <h3> Nework Info:: </h3> <h4> chainId: {networkData.chainId} Name: {networkData.name}</h4>
                    
                        <h3>-------------------------------</h3>
                        <h3> Add funds to custodian wallet </h3>
                        Custodian Wallet ID: <input type="text" placeholder='wallet address' size='40' value={custodianWallet} onChange={(e) => setCustodianWallet(e.target.value)}/>
                        <h4> </h4>
                        Funds to Add: <input type="text" placeholder='amount' size='5' value={transferAmount} onChange={(e) => setTransferAmount(e.target.value)}/>
                        <h4> </h4>
                        <Button variant="primary" name='transfer funds' value={'Transfer Funds'}  onClick={transferFunds}>Add Funds</Button>
                        <h5>{txnStatus}</h5>
                        <h4> Custodian Wallet Balance : {custodianWalletBalance} </h4>
                    
                </div>
                :
                <h3>No wallet connected, pls connect first.</h3>
          }</span>
        </div>
    </div>
  )
